...
# ──────────────────────────────────────────
BOT_TOKEN      = "MTQ5MTkwMDUxMzk2NzUzODIxNg.G3LmyG.oDoWtBaEZf0TrvsARBRl4Dkeu6B4mls_XybG9o"
...
def to_naive_utc(dt: datetime) -> datetime:
    """Converte datetime com timezone para sem timezone (UTC)."""
            ...
            if membro:
                for activity in membro.activities:
                    if isinstance(activity, (discord.Game, discord.Activity)):
                        jogo   = activity.name
                        inicio = to_naive_utc(activity.start) if activity.start else datetime.utcnow()
...
async def on_presence_update(before: discord.Member, after: discord.Member):
    ...
    # ── Começou a jogar ────────────────────────────────────────────────────────
    if jogo_novo:
        # Tenta ler o timestamp real da atividade
...
async def notificacao_hora():
    ...
    for uid in MONITORIZAR:
        ...
        # Só notifica se tiver pelo menos 1 hora na sessão atual
